...
StageProgressWidget::StageProgressWidget(QWidget *parent) : QFrame(parent)
{
   ...
   // _stage_name->setText(_stage->getStageName());
    ...
    _progress = new QProgressBar(_progress_box);
    _monheader = new MonHeaderWidget(_progress_box);
    ...
    _progress->setGeometry(_curr_start_date.dayOfYear()*_size_factor, 0, length, _vert_size);
    ...
    {
        _progress->setStyleSheet("QProgressBar { border: 1px solid grey; background-color: rgb(180, 180, 180); text-align: middle; border-radius: 3px;} \
    ...
    _widget_layout->addWidget(_progress_box);

    _start_date_label = new QLabel((_stage->getStartDate().toString("dd.MM.yyyy")),_startfinish_labels_box);
    _start_date_label->setGeometry(_curr_start_date.dayOfYear()*_size_factor, 0,100,(int)_vert_size/2);
    _start_date_label->setStyleSheet("background-color: rgb(240, 240, 240); width: 3px; border: 0px solid black;");

    _finish_date_label = new QLabel((_stage->getFinishDate().toString("d.MM.yyyy")),_startfinish_labels_box);
    _finish_date_label->setGeometry(_curr_start_date.dayOfYear()*_size_factor+length, 0, 100,(int)_vert_size/2);
